...
SELECT order_id, customer_name, book_name, sales_price, order_date
...
-- 1) 조인
SELECT C.CUSTOMER_NAME AS 이름 , SUM(O.SALES_PRICE) AS 총구매액
...
SELECT C.CUSTOMER_NAME,(SELECT SUM(O.SALES_PRICE) FROM orders O WHERE C.CUSTOMER_ID = O.CUSTOMER_ID)
  FROM customers 
...
SELECT C.CUSTOMENR_ID, C.CUSTOMER_NAME, SUM(D.SALES_PRICE)
  FROM customers C, orders O
 WHERE c.customer_id = o.customer_id
 GROUP BY c.customer_id, c.customer_name;
...
SELECT C.CUSTOMER_ID, C.CUSTOMER_NAME, SUM(ORDER_ID)
...
--2. ORDERS 테이블에 해당 정보를 INSERT한다. 주문일자는 현재 날짜로 한다.
--3. CUSTOMERS 테이블에 POINT를 UPDATE 한다.(총주문액(SALES_PRICE * AMOUNT)의 10% 적립)(총주문액(SALES_PRICE * AMOUNT)의 10% 적립)
...
BEGIN
    INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, BOOK_ID, SALES_PRICE, AMOUNT, ORDER_DATE)
           VALUES (ORDERS_SEQ.nextval, V_CUSTOMER_ID, C_BOOK_ID, (SELECT TRUNC(PRICE*0.9) FROM BOOKS WHERE BOOK_ID = V_BOOK_ID), V_AMOUNT,SYSDATE);
    UPDATE customers 
           SET POINT = POINT + (SELECT TRUNC(SALES_PRICE * AMOUNT * 0.1) FROM orders WHERE ORDER_ID = (SELECT MAX(ORDER_ID) FROM ORDERS))
           WHERE CUSTOMER_ID = V_CUSTOMER_ID;
    UPDATE BOOKS SET STOCK = STOCK - V_AMOUNT WHERE BOOK_ID = V_BOOK_ID;
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('에러메세지' || SQLERRM); --SQLERRM : SQLCODE에 해당하는 오류메세지 반환
    ROLLBACK;
END ORDER_PORC;
